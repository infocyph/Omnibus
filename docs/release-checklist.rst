...
#. Before upgrading from the previous major, drain, explicitly migrate, or
   discard old queue, failure, and workflow records. The old workflow states
   and APIs are intentionally unsupported.
...
* handled workflow redelivery does not re-run business code;
* stale workflow dispatch claims cannot settle newer ownership;
