...
Malformed JSON, oversized payloads, and unknown message/stamp aliases are
reserved as poison deliveries. The consumer rejects each one exactly once and
stores its bounded raw payload and decode error. It never returns a poison
payload to the ready queue.
...
`FailureManager` retries only a decoded failure. The failure record is removed
after the sender accepts the retry. A raw/unknown payload remains inspectable
but cannot be retried until the required codec is registered. `forget`,
`flush`, and `prune` delegate to the selected failure store.

`size()` means currently visible/ready depth, not delayed or actively reserved
messages. A provider may declare that value approximate.
...
The consumer's cooperative deadline is not a hard interrupt. A handler may
inspect `CancellationStamp::token`, and `DeadlineExecutionScope` checks again
after the handler returns. Console remains responsible for hard process
