    ...
    public static function statements(
        ...
        return match ($driver) {
            'mysql' => [
                ...
                "CREATE TABLE {$workflowItem} (workflow_id CHAR(26) NOT NULL, item_id CHAR(26) NOT NULL, item_index INT UNSIGNED NOT NULL, queue_name VARCHAR(191) NOT NULL, payload MEDIUMTEXT NOT NULL, item_status VARCHAR(16) NOT NULL CHECK (item_status IN ('pending', 'dispatched', 'succeeded', 'failed', 'cancelled')), PRIMARY KEY (workflow_id, item_id), UNIQUE KEY (workflow_id, item_index), FOREIGN KEY (workflow_id) REFERENCES {$workflow} (id) ON DELETE CASCADE)",
            ...
            'pgsql' => [
                ...
                "CREATE TABLE {$workflowItem} (workflow_id CHAR(26) NOT NULL REFERENCES {$workflow} (id) ON DELETE CASCADE, item_id CHAR(26) NOT NULL, item_index INTEGER NOT NULL CHECK (item_index >= 0), queue_name VARCHAR(191) NOT NULL, payload TEXT NOT NULL, item_status VARCHAR(16) NOT NULL CHECK (item_status IN ('pending', 'dispatched', 'succeeded', 'failed', 'cancelled')), PRIMARY KEY (workflow_id, item_id), UNIQUE (workflow_id, item_index))",
                "CREATE INDEX {$workflowItemIndex} ON {$workflowItem} (workflow_id, item_status, item_index)",
            ],
            'sqlite' => [
                ...
                "CREATE TABLE {$workflowItem} (workflow_id TEXT NOT NULL REFERENCES {$workflow} (id) ON DELETE CASCADE, item_id TEXT NOT NULL, item_index INTEGER NOT NULL CHECK (item_index >= 0), queue_name TEXT NOT NULL, payload TEXT NOT NULL, item_status TEXT NOT NULL CHECK (item_status IN ('pending', 'dispatched', 'succeeded', 'failed', 'cancelled')), PRIMARY KEY (workflow_id, item_id), UNIQUE (workflow_id, item_index))",
                "CREATE INDEX {$workflowItemIndex} ON {$workflowItem} (workflow_id, item_status, item_index)",
            ],
